...
src_dir = r"d:\website\raw-assets\projects\marketing\campaign 2"
gallery_dir = r"d:\website\assets\img\gallery\ac-cooling"
...
if os.path.exists(thumb_src):
    ...
    shutil.copy(thumb_src, os.path.join(projects_img_dir, "ac-cooling.png"))
else:
    print("Thumbnail not found at", thumb_src)
...
for slide in os.listdir(os.path.join(gallery_dir, "presentation")):
    if slide.endswith(".png"):
        gallery.append(f"assets/img/gallery/ac-cooling/presentation/{slide}")

for post in os.listdir(os.path.join(gallery_dir, "social-media")):
    if post.endswith((".png", ".jpg", ".jpeg")):
        gallery.append(f"assets/img/gallery/ac-cooling/social-media/{post}")
